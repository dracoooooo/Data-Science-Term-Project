...
public class Cache {	//
	...
	public int fetch(String sAddr, int len) {
		...
		int map = mappingStrategy.map(blockNO);
		if(map == -1){
			map = mappingStrategy.writeCache(blockNO);
		}
		return map;
	}
	...
	private char[] helper(String eip, int len, char[] writeData){
		...
		while (addr < upperBound) {
			...
			if(writeData == null){
				...
				System.out.println("write: " + rowNO);
			}
			addr += nextSegLen;
		}
		return data;
...
//				cache.get(rowNO).setTimeStamp(0L);
			}
		}
	}
	...
	public class CacheLine {
		...
		public void setValidBit(boolean b){ this.validBit = b; }

		public boolean isDirty() { return this.dirty; }

		public void setDirty(boolean b){ this.dirty = b;}
		...
		public void setDate(char[] data){ this.data = data;}
